...
|---|----------|------|-------------|--------|
| 1 | Critical | `backend/app.py` | Directory traversal in SPA fallback — add `.resolve()` + `.is_relative_to()` check | ✅ DONE |
| 2 | High | `desktop_launcher.py` | `/api/app/quit` doesn't close pywebview window — register `window.destroy()` shutdown callback | ✅ DONE |
| 3 | Medium | `icloud_sorter.spec` | Fragile `hiddenimports` — remove unnecessary `backend.*` entries | ✅ DONE |
...
| 5 | Low | `backend/config.py` | Registry key fallback — add `Software\Apple Inc.\Internet Services` lookup | ✅ DONE |
